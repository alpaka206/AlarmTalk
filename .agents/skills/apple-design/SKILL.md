...
이 저장소는 Android Compose 앱이다. 웹 예시는 원리로 번역: 속도 이어받기 → `Animatable.animateTo(initialVelocity = velocity)`(draggable `onDragStopped`이 velocity 제공), 1:1 트래킹 → `snapTo`, 중단 가능성 → Animatable은 새 호출이 기존 애니메이션을 자연스럽게 대체, 커밋/복귀 판단은 위치보다 속도 부호 우선.
