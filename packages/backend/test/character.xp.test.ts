...
describe('POST /characters/xp', () => {
  ...
  it('여유 내 alarm_completed → 201, XP +5, affection +2', async () => {
    ...
    mockDB.pushResult([baseCharacter({ xp: 5, affection: 2, daily_xp: 5 })]); // #7 loadOrCreateCharacter (refresh)
  ...
  it('날짜 바뀌면 daily_xp 리셋 후 재지급', async () => {
    ...
    expect(body.grant.granted_xp).toBe(5);
    expect(body.grant.capped).toBe(false);
    expect(body.character.xp).toBe(205);
  ...
  it('alarm_dismissed → XP -5, affection 0 이어도 로그는 남음', async () => {
    mockDB.pushResult([{ id: 'user-pk-1' }]); // #1
    mockDB.pushResult([baseCharacter({ xp: 20, daily_xp: 20 })]); // #2
    mockDB.pushResult([], 1); // #3 UPDATE characters
    mockDB.pushResult([], 1); // #4 INSERT log
    // alarm_dismissed does NOT trigger streak update, no ensureStatsRow/UPDATE stats
    mockDB.pushResult([baseCharacter({ xp: 15, daily_xp: 20 })]); // #5 loadOrCreateCharacter (refresh)
  ...
  it('alarm_dismissed 감점은 현재 레벨 최소 XP 아래로 내려가지 않음', async () => {
    mockDB.pushResult([{ id: 'user-pk-1' }]); // #1
    mockDB.pushResult([baseCharacter({ level: 2, xp: 100, daily_xp: 20 })]); // #2
    mockDB.pushResult([], 1); // #3 UPDATE characters
    mockDB.pushResult([], 1); // #4 INSERT log
    mockDB.pushResult([baseCharacter({ level: 2, xp: 100, daily_xp: 20 })]); // #5 refresh
    mockDB.pushResult([]); // #6 loadStats
    mockDB.pushResult([]); // #7 loadAchievements
