...
const ANDROID: AppVersionPolicy = {
  ...
  //
  // 22 는 건너뛰고 23 으로 올린다 — 1.2.2 때 이 값을 안 올려 22 가 권장 기준이 된 적이
  // 없다. 지금 올리는 이유는 1.2.3 이 **고쳐 둔 알람 시각이 조용히 되돌아가던** 버그를
  // 고치기 때문이다. 그건 사용자가 스스로 알아채기 어렵고(고친 줄 알고 잠든다), 못 일어나는
  // 결과로 이어진다.
  //
  // ⚠️ 배포 순서: **1.2.3(versionCode 23)이 Play 에 올라간 뒤** 이 변경을 main 에 머지할 것.
  // 먼저 나가면 22 이하 사용자에게 받을 것이 없는 업데이트 안내가 뜬다(차단은 아니다 —
  // Play 에 새 버전이 없으면 인앱 업데이트는 no-op 이라 배너만 헛돈다).
  latest: 23,
