...
///
/// 왜 미리 받나: 알람을 만들 때 네트워크가 없으면 테마를 고를 수가 없고, 울릴 시각에
/// 파일이 없으면 그 회차가 조용히 비어 버리므로 쓰기 전에 받아 둔다.
///
/// ⚠ 예전 주석은 "울릴 때마다 클립을 순차 회전한다" 고 적었는데 **iOS 에는 그 회전이
/// 없다**(안드로이드에만 있다 — `FreeBucketSettings.swift` 주석 참조). 전부 받아 두는
/// 이유는 회전이 아니라 오프라인에서도 테마를 고르고 울릴 수 있게 하기 위해서다.
///
