...
// 실제 구현은 `AlarmTalkAPI` 의 extension 으로 conform.
// `Sendable` — MainActor 격리된 store 가 async 컨텍스트로 api 를 캡처할 때 race
// 경고를 피하기 위해. 실제 conformer 인 `AlarmTalkAPI` 는 `@unchecked Sendable`.
...
//                              `AlarmTalkApp.swift` `.task(id:)` / `.onChange(of: scenePhase)`.
//
// 멱등 nonce 정책:
...
final class CharacterEventStore: ObservableObject {
    ...
    /// 초기 로드. `AlarmTalkApp` `.task` 에서 1회 호출해 디스크 → 메모리 hydrate.
