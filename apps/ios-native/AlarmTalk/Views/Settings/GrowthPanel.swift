...
private struct CharacterSyncStatusView: View {
    ...
    var body: some View {
        ...
        .foregroundStyle(needsCheck ? AlarmTalkTheme.error : AlarmTalkTheme.primary)
        .padding(.horizontal, 14)
        .padding(.vertical, 12)
        .background((needsCheck ? AlarmTalkTheme.error : AlarmTalkTheme.primary).opacity(0.08))
...
private func eventXpColor(_ event: String) -> Color {
    switch event {
    case CharacterEventType.alarmCompleted.rawValue:
        return AlarmTalkTheme.primary
    case CharacterEventType.alarmSnoozed.rawValue, "alarm_dismissed":
        return AlarmTalkTheme.error
    default:
        return AlarmTalkTheme.textSecondary
    }
}
