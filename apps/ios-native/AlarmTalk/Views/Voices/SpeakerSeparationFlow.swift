...
struct SpeakerSeparationFlow: View {
    ...
    private func speakerCard(speaker: VoiceSpeakerSegment, index: Int) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                if let label = speaker.label as String?, !label.isEmpty, label != "Speaker A" {
                    Text(label).font(.caption).foregroundStyle(AlarmTalkTheme.textSecondary)
                }
                Spacer()
            ...
            HStack(spacing: 8) {
                ...
                Text(draftStatusLabel(draftState))
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(draftState.status == .failed ? AlarmTalkTheme.error : AlarmTalkTheme.textSecondary)
                Spacer(minLength: 0)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 10)
                .stroke(draftState.isReady ? AlarmTalkTheme.primary.opacity(0.7) : AlarmTalkTheme.outline, lineWidth: draftState.isReady ? 1.5 : 1)
        )
    ...
    private func stepLabel(num: Int, title: String) -> some View {
        HStack(spacing: 8) {
            Text("\(num)")
                ...
                .background(AlarmTalkTheme.primary, in: Circle())
    ...
    private func draftStatusIcon(_ state: SpeakerDraftState) -> some View {
        ...
        case .cloning:
            systemName = "waveform"
            color = AlarmTalkTheme.textSecondary
        case .synthesizing:
            systemName = "speaker.wave.2.fill"
            color = AlarmTalkTheme.textSecondary
        case .ready:
            systemName = "checkmark.circle.fill"
            color = AlarmTalkTheme.primary
        case .failed:
            systemName = "exclamationmark.triangle.fill"
            color = AlarmTalkTheme.error
        }
