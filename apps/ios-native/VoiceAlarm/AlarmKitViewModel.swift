...
final class AlarmKitViewModel: ObservableObject {
    ...
    func schedule(record: LocalAlarmRecord, store: LocalAlarmStore) async {
        ...
        do {
            ...
            statusMessage = "Scheduled \(record.label)"
        } catch {
    ...
    private func makeSchedule(_ record: LocalAlarmRecord) -> Alarm.Schedule {
        ...
        if record.repeatWeekdays.isEmpty {
            ...
            recurrence = .weekly(record.repeatWeekdays.compactMap(localeWeekday))
        }
    ...
    private func makeConfiguration(
        ...
        let snoozeDuration = Alarm.CountdownDuration(preAlert: nil, postAlert: 5 * 60.0)
