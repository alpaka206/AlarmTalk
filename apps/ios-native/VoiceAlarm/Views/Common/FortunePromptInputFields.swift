    ...
    private func choiceButton(label: String, selected: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(label)
                ...
                .foregroundStyle(selected ? VoiceAlarmTheme.text : VoiceAlarmTheme.textSecondary)
                .frame(maxWidth: .infinity)
                .frame(height: 42)
                .background(selected ? VoiceAlarmTheme.primary.opacity(0.18) : VoiceAlarmTheme.surfaceVariant.opacity(0.7))
                .overlay(
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                        .stroke(selected ? VoiceAlarmTheme.primary.opacity(0.55) : VoiceAlarmTheme.outline, lineWidth: 1)
                )
    ...
    private func selectorButton(
        ...
        Button(action: action) {
            HStack(spacing: 10) {
                ...
                Text(title)
                    .font(.subheadline.weight(.semibold))
                    .foregroundStyle(placeholder ? VoiceAlarmTheme.textSecondary : VoiceAlarmTheme.text)
                Spacer()
            ...
            .background(VoiceAlarmTheme.surfaceVariant.opacity(0.7))
            .overlay(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
