 ...
 *
 * `src` 는 이 탭이 들고 있는 mp3 의 Blob URL 이다(`event-api.ts`). 같은 출처라 `download` 의
 * 파일명이 그대로 먹는다 — 안드로이드는 바로 받고, iOS Safari 는 파일 앱에 저장한다. 없으면
 * "그냥 다운로드" 는 눌리지 않는다 — 눌렀는데 아무 일도 없는 버튼을 두지 않는다.
 */
