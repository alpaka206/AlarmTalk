...
- **리뷰 수정 중 빌드·테스트·CI를 임의로 실행하지 않는다**(2026-09-13 사용자 재확인).
  실행은 **`develop → main` 머지가 끝난 뒤**로 모은다. CI 실패 원인 확인 요청만으로
  빌드·테스트·CI 재실행까지 허용된 것으로 해석하지 않는다. 그 전 PR에서는 요청된 리뷰 수정과
  필요한 스펙·회귀 테스트 작성/기록만 하고, 자체 전체 점검·추가 리뷰 요청을 시작하지 않는다.
  `ci` 라벨도 자동으로 붙이거나 재부착하지 않는다. 실행하지 않은 검사는 통과로 적지 않는다.
  현재 워크플로에는 라벨/브랜치 push 트리거가 남아 있고 보호 브랜치의 필수 체크도 별개다.
  머지가 막혀도 임의로 CI를 실행하거나 보호 설정을 우회/변경하지 말고 사용자에게 알린다.
