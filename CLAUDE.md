...
| 빠진 권한 | 실제 결과 | 어떻게 말하나 |
| --- | --- | --- |
| 알림 | 울림 화면을 띄울 통로가 없다 → **안 울린다** | 헤드라인이 남은 시간 대신 "울리지 않아요" |
| 정확한 알람 | `setAndAllowWhileIdle` 폴백으로 **울리되 수 분 늦을 수 있다** | 남은 시간은 그대로, 배너로 "제때 울리지 않을 수 있어요" |
| 잠금 화면 | 소리는 나되 **잠금 화면을 못 덮는다** | 남은 시간은 그대로, 배너로 잠금 화면 안내 |

셋을 묶어 전부 "안 울린다" 고 하면 멀쩡히 울릴 알람을 사용자가 없는 것으로 믿는다
(Codex #671 P2 지적). 게이트 제목은 정책을 말하므로 권한별로 "…권한을 허용해야 알람을
설정할 수 있어요" 로 통일하고, 홈 화면은 위 표대로 **사실**을 말한다.
