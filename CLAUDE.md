...
- **'전체 동의' 의 범위는 제품 결정이다 — 법이 정해 주지 않는다.**
  - **법이 실제로 요구하는 것**(개인정보보호법 제22조): ① 동의 사항을 **구분하여 알리고
    ...
    **구분 표시**. **"전체 동의 버튼이 선택 항목까지 켜는 것" 자체를 금지하는 조문은 없다** —
    국내 다수 앱이 그렇게 한다.
  - **우리가 지금 택한 것**: 마스터 행은 **필수 전용**이다. 거절했던 사람이 재동의 화면에서
    한 번의 탭으로 마케팅을 다시 켜게 되는 것을 피하려는 보수적 선택이다.
...
### 목소리 게이트는 **세 상태**다 — 둘로 가르지 말 것 (양 앱)

2026-08-07 사용자 문의로 드러났다: **"로그인도 했고 쿠폰도 등록했는데 '로그인이 필요하다'
모달이 뜬다."** 원인은 상태가 셋인데 분기가 둘이었던 것이다.

| 상태 | 무엇이 문제인가 | 무엇을 말하나 | 어디로 보내나 | 쿠폰 입력 |
| --- | --- | --- | --- | --- |
| 비로그인 | 세션이 없다 | "로그인이 필요해요" | 로그인 | **붙이지 않는다** — 등록할 계정이 없다 |
| 로그인 + 무료 | 이용권이 없다 | "유료 이용권이 필요해요" | 결제 | **여기서만** 뜻이 있다 |
| 로그인 + 유료 | **플랜 문제가 아니다** — 기본(시스템) 목소리는 준비된 클립만 말한다 | "기본 목소리로는 직접 입력을 쓸 수 없어요" | **목소리 등록** | 붙이지 않는다 — 이미 유료다 |

- ⚠ **`freeVoiceTier` 의 `else` 를 '비로그인' 으로 읽지 말 것.** 그 값은
  `authSession != null && !hasPaidVoiceAccess(...)` 라, 부정에는 비로그인뿐 아니라
  **로그인한 유료 사용자**가 함께 들어간다. 판정은 `VoiceGateReason`(Android) /
  `PlanAccess`(iOS) 처럼 **세 갈래 열거형**으로 한다.
- **유료가 이 게이트에 닿는 길은 실재한다.** 기본 목소리를 고르면 유료여도 문구가
  '날씨+약' 으로 제한되고, 그 pane 의 잠긴 '직접 입력' 이 게이트를 연다.
- **버튼은 상태마다 다르다.** '쿠폰이 있어요'·'이용권 보기' 는 *이용권이 없어서* 막힌
  경우에만 뜻이 있다. 다른 상태에 붙이면 눌러도 아무 일이 없거나(유료) 넣을 계정이
  없다(비로그인).

### 기본(시스템) 목소리 제한은 **유료에게도** 걸린다 — 판정은 `OR` (양 앱)

기본 목소리는 미리 만들어 둔 클립만 말할 수 있다. 그래서 문구를 무료 버킷
('날씨+약')으로 제한하는 조건은 **플랜과 목소리 종류의 OR** 다:

```
restrictToWeatherMedication = freeVoiceTier || 시스템보이스선택됨
```

- ⚠ **`&&` 로 쓰면 유료 사용자가 기본 목소리에 직접 입력 문구를 붙일 수 있다** — 저장은
  되는데 그 문구를 말할 방법이 없는 알람이 된다. iOS 가 실제로 `&&` 였다(2026-08-07 수정).
- ⚠ **한 곳만 고치지 말 것.** 안드로이드는 이 플래그를 **렌더·상태강제·저장검증** 세
  갈래에서 함께 본다(`AlarmEditorScreen.kt` 의 `restrictToWeatherMedication`,
  iOS 는 `AlarmEditorSheet.restrictToWeatherMedication`). 렌더만 맞추면 화면과 저장이
  어긋나 '보이는 대로 저장되지 않는' 상태가 된다.
- **문구도 상태마다 다르다.** 무료라서 막힌 것과 기본 목소리라서 막힌 것은 다른 사실이다 —
