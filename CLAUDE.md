...
- **리뷰 수정은 필요한 코드·스펙·회귀 테스트 작성 뒤 커밋·푸시까지 하고 종료한다**
  (2026-09-13 사용자 명확화). 별도 요청 없이 로컬 빌드·테스트·자체 전체 점검·추가 리뷰
  요청을 시작하지 않는다. 로컬 빌드는 **`develop → main` 머지가 끝난 뒤** 릴리스 작업에서 다룬다.
  **원격 CI는 실행해도 된다.** 로컬 빌드 제한을 CI에 적용해 취소하거나, CI 완료를 기다리며
  작업을 계속하지 않는다. 기존 워크플로와 보호 브랜치의 필수 체크는 유지한다.
  `ci` 라벨은 자동으로 붙이거나 재부착하지 않는다. 실행하지 않은 검사는 통과로 적지 않는다.
  보호 설정을 우회하거나 변경하지 않는다.
