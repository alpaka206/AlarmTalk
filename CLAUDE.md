...
- **안드로이드는 울릴 때 기기 알람 볼륨을 맞춘다**(`AlarmStreamVolume`). `MediaPlayer.setVolume`
  ...
  ⚠ **그때 넘기는 퍼센트는 그 알람이 실제로 낼 소리의 슬라이더다**(2026-08-27 실기기 재현).
  재생 방식과 무관하게 알람음 퍼센트를 넘기면, 목소리 알람은 그 슬라이더가 **화면에 없어
  늘 100** 이라 기기 볼륨이 최대로 덮어써진다 — 사용자가 목소리 크기를 10% 로 낮추고 기기
  알람 볼륨까지 내려 둬도 크게 울린다("목소리 크기가 안 먹는다"). 위 「곱하지 말 것」을
  스트림 쪽에서도 지키는 것이다.
