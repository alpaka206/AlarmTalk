...
> **신규** 가족 그룹부터 5인 정원 적용. (`plan_groups` 는 생성 시점 스냅샷이고, ⚠ prod DB 초기화는 **하지 않는다**(2026-08-01 확정, CLAUDE.md). 그래서 마이그레이션 #52 이전에 만들어진 6인 그룹이 남아 있다면 그 값이 그대로 유지된다 — 5인으로 내려야 한다면 제자리 마이그레이션을 따로 추가한다.)
