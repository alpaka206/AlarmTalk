...
또한 **번들 스톡클립 경로(2안)를 살려 둬라.** 옛 코드는 1안(`Library/Sounds`) 기본 +
2안 폴백인데, **2안 경로를 지우거나 약화시키지 마라.** 30초 초과·트랜스코드 실패에서도 필요하다.

> **2026-08-18 갱신**: 1안은 **실기기에서 동작한다**(잠금화면 포함). "Apple 이 known issue 로
> 답해서 못 믿는다" 던 서술은 반증됐다 — 우리 증상의 원인은 `.named` 에 **확장자를 안 붙인
> 우리 버그**였다. 자세한 것은 `분석/03-플랫폼제약.md` 의 '커스텀 오디오' 절.
