# User Manual

> [English](README.md) · [한국어](README.ko.md) · [日本語](README.ja.md)

End-user manual for **Naro**, an OS-native voice alarm app.

## 1. What Naro does

Naro rings an alarm at a scheduled time using a voice you picked — your own recording, an uploaded clip, a voice shared by family or a partner, or a voice generated by an AI clone.

- Works without a network. Rings on flight mode, lock screen, and battery saver.
- The alarm is a real alarm, not a notification. It rings even if your phone screen is off.

## 2. Before you install

- Android 8.0 or newer.
- An email account or a Google account.
- Notification, exact-alarm, and full-screen-intent permissions.
- Microphone permission if you plan to record your own voice.

## 3. First-time setup (5 minutes)

1. **Sign up**
   - Use email/password (8+ characters, letters and digits), or continue with Google.

2. **Grant permissions**
   - Three permission cards appear: notification, exact alarm, full-screen. Allow each one.
   - All three are required so the alarm rings even on the lock screen.

3. **Create your first voice**
   - Voices tab → "+ New voice".
   - Allow microphone access → tap Record.
   - Record up to 30 seconds (e.g., "Good morning, time to wake up").
   - Give it a name (e.g., "Mom's voice") and save.

4. **Create your first alarm**
   - Alarm tab → "+ New alarm".
   - Pick the time and repeat days.
   - Choose mode = "Alarm + voice" (recommended): a default alarm tone plays first; when you tap stop, the voice plays once.
   - Pick the voice profile you just created and type the message to read.
   - Save.

5. **Test ring**
   - Set an alarm for 1–2 minutes from now and wait.
   - Try it with the screen locked, with the app closed, and in airplane mode.

## 4. If permissions aren't granted

- **Notification**: Settings → Apps → Naro → Notifications → "Allow notifications".
- **Exact alarm**: Settings → Apps → Naro → "Alarms & reminders" → enable. The menu path varies a little per manufacturer.
- **Full-screen alerts**: On Android 14+ there is a dedicated menu — Settings → Apps → Naro → "Full-screen notification access".
- **Battery optimization**: Some manufacturers (Samsung One UI, Xiaomi MIUI) require Naro to be excluded from battery optimization for reliable rings.

The Account tab shows the current permission state and links straight to the system settings.

## 5. Connecting family or a partner

### Inviting (Owner)

1. Subscribe to the family or couple plan.
2. People tab → Family group → "Make invite code".
3. A 6-digit code appears. It expires in 10 minutes.
4. Share it through any messenger, SMS, or read it aloud, or use the deep link.

### Joining (Member)

1. Install Naro and sign in.
2. Account → Subscription / Vouchers → "Register code".
3. Enter the 6 digits and confirm.
4. Once joined, shared voices are available for any alarm.

### Using a shared voice for an alarm

- New alarm → mode "Voice" → audio source "Voice profile" → Shared voices tab.
- Type the message and save.

## 6. Tips

- **Be considerate**: don't test alarms at unsocial hours; neighbors hear them too.
- **Heavy sleeper**: schedule three repeating alarms 5–10 minutes apart on weekdays.
- **Volume**: alarm volume follows the system Alarm category. Settings → Sound → Alarm volume.
- **Vibration**: "Default" for light sleep, "Strong" for deep sleep.

## 7. Subscriptions and codes

### Plans

- **Free**: 1 voice profile, 5 TTS / day.
- **Personal**: 2 voice profiles, 30 TTS / day.
- **Couple / Family**: voice sharing for 2–6 members.

### Codes

- `VA-XXXX-XXXX-XXXX` → voucher: activates a paid plan for a period without payment.
- 6-digit numeric → family group join.

### Pause / cancel

- Payments renew monthly. Account → Subscription → "Pause".
- Your subscription stays active until the next billing date, then drops to Free automatically.

## 8. FAQ

### Q. My alarm did not ring once.
- 99% of the time it is a permission issue. Account → permission state.
- If permissions are fine, exclude Naro from battery optimization and turn off "auto-close unused apps" in your manufacturer's settings.
- If it still does not ring, contact support.

### Q. If the server is down, do my alarms still ring?
- Yes. Naro schedules alarms locally on the device, so server outages do not affect ring behavior.
- Creating new TTS or fetching shared voices is paused during an outage.

### Q. Is my recorded voice uploaded to a server?
- Yes — uploads are required for voice cloning and TTS generation. The data is stored in a private cloud bucket and is deleted when you delete your account.
- It is only shared inside your family / partner group. External download is disabled by design.

### Q. What if a family member uses my voice in an unwanted way?
- You can remove your voice profile from the group. If you need help, Settings → Support.

### Q. Are there more built-in alarm tones?
- In v1.0 the default tone is a single short pattern. More built-in tones come in a later update.

## 9. Switching to a new device

1. Install Naro on the new device → sign in with the same account.
2. Account → "Sync now".
3. Alarm list, voice profiles, and subscription are restored.
4. Voice clips that were recorded locally on the old device are not transferred automatically. Re-record on the new device if needed. Shared voices are already available.

## 10. Support

- Email: see `SECURITY.md` for security-only contact. For product support, use the support link inside the app.
- When reporting an issue, please include:
  - App version (Account → App info)
  - Device manufacturer / model / Android version
  - When the issue happened
  - Steps to reproduce, if possible

## 11. Glossary

| Term | Meaning |
|---|---|
| Voice profile | A cloned voice made from a recording or upload. |
| TTS | Text-to-Speech. Turns text into audio. |
| Shared voice | A voice profile created by another member of your family / partner group. |
| Alarm mode | "Alarm only", "Voice only", or "Alarm + Voice". |
| Snooze | Postpones the alarm for a few minutes. |
| Invite code | 6-digit numeric code for joining a family group. 10-minute TTL, single use. |
| Voucher code | `VA-XXXX-XXXX-XXXX` from a promotion or partner. |
| Character | A 4-stage in-app companion that grows as you complete alarms. |
