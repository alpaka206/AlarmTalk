...
- **무엇이 이기는가는 종류에 따라 다르다** — 이 구분이 없어서 이 문서와
  [`spec/README.md`](spec/README.md) 가 한동안 정반대를 말했다(2026-09-02 정정).
  - **규범**(제품이 어떻게 **동작해야** 하는가) → **스펙이 이긴다.** 구현이 다르면 구현이 틀렸다.
    예: "재생 방식은 둘뿐", "미체크 ≠ 철회". 사는 곳은 `spec/`.
  - **서술**(코드가 어떻게 **생겼는가**) → **코드가 이긴다.** 문서로 옮겨 적지 말고 코드를 가리켜라.
    예: 함수 위치, 필드 이름, 파일 구조.
