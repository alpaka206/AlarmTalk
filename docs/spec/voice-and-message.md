...
**모든 사용자가 같은 여섯 줄을 본다**: 기본 인사말 · 날씨 · 운세 · 사랑 · 약 · 직접 입력.
목소리를 무엇으로 골랐든, 유료든 무료든 같다.

목록에서 줄이 빠지는 사유는 **하나뿐**이다 — **그 (목소리 · 언어) 로 구워 둔 클립이
서버에 아직 없을 때.** 판정은 스톡 매니페스트(`GET /tts/stock-clips`)와의 교차이고,
운영이 새 카테고리를 시딩하면 **앱을 고치지 않아도** 줄이 나타난다.
...
- 받는 대상(기본 목소리) = 기본(시스템) 목소리 **전부** × **기기 언어 하나** × 알람에
  쓰는 카테고리 **다섯**(greeting 1 · weather 9 · fortune 5 · love 3 · medication 2)
  = 4 × 20 = **80개**
- 언어를 하나로 좁힌다 — 앱은 한 번에 한 언어만 쓰고, 언어를 바꾸면 다시 돌아 채운다
- ⚠ **다섯을 전부 받는다**(2026-09-02). §2 대로 기본 목소리도 다섯 종류를 고를 수 있게
  됐으므로, 안 받는 종류가 있으면 **고를 수는 있는데 오프라인에서 소리가 안 나는** 알람이
  생긴다. 목록이 늘면 받는 것도 같이 늘어야 해서 두 집합을 한 곳에서 유도한다
  (`FreeBucketOrder` / `FreeBucket.order`).
- greeting 은 앱에도 내장돼 있지만(목소리 미리듣기용) 알람 경로는 서버 클립을 messageId 로
  캐시하므로 여기서도 받는다
