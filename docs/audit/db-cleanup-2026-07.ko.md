...
- ~~유료 음성 보관 유예 30일 → 3일~~ **되돌림(30일 유지)**. 앱의 해지 안내가 "만든 목소리는
  삭제되지 않고 잠기며, 다시 이용권을 등록하면 그대로 다시 쓸 수 있어요"라고 약속하는데,
  3일로 줄이면 5분 주기 정리 cron 이 그 약속보다 먼저 지워 재구독해도 되살릴 게 없어진다.
  저장 비용 때문에 줄이려면 해지 다이얼로그 카피를 함께 바꿔야 한다(별도 결정).
- 미사용 라우트 정리 (§9-A)
