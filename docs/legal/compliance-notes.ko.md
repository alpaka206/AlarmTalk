...
- **민감·별도 동의는 가입 조건에서 뺀다**: 음성 생체정보(`voice_biometric`)와 국외 이전
  (`overseas_transfer`)은 목소리 등록·AI 음성 생성·동적 문구 등 **그 처리가 실제로 발생하는
  시점**에 받는다. 제22조는 선택·별도 동의를 하지 않았다는 이유로 재화·서비스 제공을 거부하지
  못하게 하므로, 목소리를 등록하지 않을 이용자에게 생체정보 처리 동의를 가입 조건으로 요구할 수
  없다. 이는 "동의는 개인정보 처리가 필요한 시점에 받는다"는 위 원칙과도 일치한다.
