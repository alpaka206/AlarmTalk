...
- 가입 선택 동의: ④ 마케팅·광고성 정보 수신(`marketing`) — 미동의해도 가입·이용에 제한 없음
- 만 14세 미만 가입 불가(연령 확인). → 법정대리인 동의(제22조의2) 절차를 두지 않기 위함.
- **민감·별도 동의도 가입 시 함께 받는다**(2026-07-30 결정): 음성 생체정보(`voice_biometric`)와
  국외 이전(`overseas_transfer`)을 가입 동의 화면의 필수 항목에 포함한다
  (`REQUIRED_CONSENT_TYPES` = 필수 5종).
  - 근거: 앱의 핵심이 목소리 알람이라 음성 처리는 서비스 본질에 해당하고, 기능을 쓰려는
    순간마다 동의 모달을 띄우면 그 자리가 가장 거부감이 크다는 제품 판단.
  - **남는 위험(기록해 둔다)**: 목소리 클론은 유료 전용 기능이라 무료 이용자는 사용할 수
    없는데, 그 기능의 생체정보 동의가 가입 조건이 된다. 제22조 제5항은 선택·별도 동의를 하지
    않았다는 이유로 재화·서비스 제공을 거부하지 못하게 하므로, 이 구성이 다투어질 소지가 있다.
    문제 제기가 있으면 생체정보 동의만 목소리 등록 시점으로 되돌리는 것이 대안이다
    (해당 화면과 서버 게이트는 이미 구현돼 있어 상수 한 줄로 되돌릴 수 있다).
