...
- **법무 고지 정합성** *(2026-07-15 대조: 대부분 완료)*: 개인정보처리방침 버전 2가 사주 항목(성별/생년월일/출생 시각)·Vertex 국외 이전·FCM/PortOne 수탁을 이미 명시(§1.4·§5·개정 이력). 사주 항목은 민감정보(§23) 아님 — 일반 개인정보로 수집·이용 고지면 충분하고 국외 이전 동의는 서버 강제 중. 방침이 선언한 "GPS 비수집"은 앱의 '현재 위치 사용'(GPS) 기능·위치 권한 전면 제거로 코드가 방침과 일치하게 됨(위치정보법 비적용 유지). 잔여: 방침 개정 시 재동의 버전 bump(위 항목).
