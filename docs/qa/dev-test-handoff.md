...
원인 확인 중 자동 검증까지 실행했으나, 사용자가 빌드는 `develop → main` 머지 완료 뒤라고
재확인해 진행 중이던 원격 iOS Release 빌드를 취소했다. 재실행하지 않는다.
이미 끝난 검사와 취소 결과는 [리뷰 대응 기록](pr-730-post-merge-reviews-2026-09-13.md)의
「#737 CI 실패 복구」에 기록한다. 실행 시점 규칙은 `CLAUDE.md` 「컨벤션」을 따른다.
