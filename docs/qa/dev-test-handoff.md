...
- [ ] 실행 요청 시 `RemoteAlarmColdLoadTests`의 실제 가족 콜백→전체 동기화 회귀 3건 실행
      (첫 시간 초과/취소, 취소된 대기 호출 뒤 다음 호출, 최신 세션의 실제 HTTP)
- [ ] 실행 요청 시 `DuplicateAlarmReplacementTests`의 실제 삭제 401→세션 해제 뒤 기존/신규
      롤백 및 동시 삭제·끄기·소유자/예약/편집 변경 보존 회귀 실행
- [ ] 실행 요청 시 `billing-apple-gift-refund.test.ts`의 발급 전 환불·타 계정 제보·탈퇴·재전송·
      보존 기한·기존 표식 재검증·쓰기 롤백 회귀 실행
