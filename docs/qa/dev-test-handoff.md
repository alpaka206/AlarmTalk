...
**무엇을 하는 기능인가:** 유료 구독자가 비공개 음성 초안을 만든 뒤 고정 예시 TTS를 끝까지 듣고 **사용 확정**하면 그때 KST 기준 월 1회 등록을 소진한다. 확정된 관계·호칭과 앱 언어로 **기본 인사·날씨·운세·사랑·약** 고정 프리셋 오디오를 서버의 제한된 작업 큐가 미리 렌더하고, 앱은 완성된 버킷만 내려받아 알람 발사 때 **네트워크 없이 오프라인**으로 재생한다. 직접 입력을 고르면 별도 월 한도 안에서 요청 시 TTS를 생성한다.

**등록 상태 계약:** 초안은 공유·알람·일반 TTS에 사용할 수 없고 계정당 하나만 유지한다. 제공자 비용 우회를 막기 위해 초안 생성은 월 3회까지이며, 제공자 음성이 만들어지기 전 실패만 횟수를 복구한다. 미리듣기 완료 전 승격, 공식 음성의 초안 되돌리기, 삭제 후 같은 달 재등록, 확정 뒤 관계·호칭 변경은 서버가 거부한다. 공식 음성 삭제는 월 1회 장부를 환불하지 않는다.
