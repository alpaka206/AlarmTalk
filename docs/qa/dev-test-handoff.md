...
- [ ] 같은 단계에서: iOS 탈퇴 복구 훅 5개 테스트(세션 교체 2경우 포함), 알람 커서 순회 11건 실행
- [ ] #735 후속: 실제 DB의 앞 행/커서 ack 삭제 회귀 2건, 커서 API 계약·입력 7건 실행
- [ ] #735 후속: iOS 푸시 해제 응답 유실·강제 재등록·재시작·계정 가드·직렬화 5건 실행
- [ ] #735 재후속: 탈퇴 취소 API 멱등성 실제 DB 5건과 iOS 불확실한 복구 응답 7개 테스트(12경우) 실행
- [ ] #735 디코딩 후속: `AccountRecoveryDecodingTests` 2개 테스트(7경우) 실행 — 실제 2xx 디코딩 실패
      4종 뒤 active 재확인, 재확인이 pending/503/디코딩 실패인 경우 복구 보류(작성만 완료)
- [ ] 실기기: 푸시 해제와 탈퇴 취소 응답을 모두 유실시킨 뒤 재확인/재시도/재실행에서 가족 알람 수신 확인
- [ ] 커서 지원 백엔드가 iOS보다 먼저 배포됐는지 확인(구서버 offset 응답으로 폴백하지 않음)
