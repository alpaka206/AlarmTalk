...
- **Play**: 프로덕션 `29 (1.2.9)` 게재(2026-09-22).
- **App Store**: `1.2.8` 게재 완료(2026-09-22, `READY_FOR_DISTRIBUTION`). 애플의 '배포 준비됨' 은
  승인·출시가 끝난 상태 표시다 — 대기 중이 아니다(스토어 조회로 kr/jp/us 확인).
- **App Store `1.2.9`(빌드 6)**: 2026-09-23 심사 제출, `WAITING_FOR_REVIEW`.
  - 심사 노트는 1.2.8 의 것을 물려받되 머리말만 갈았다 — **첨부 영상은 따라오지 않는다**
    (`appStoreReviewAttachments` 가 빈 목록). 1.2.8 의 노트는 "영상이 첨부돼 있다" 로 시작했으므로
...
- [ ] iPhone 실기기에서 OS BGTask 전달·콜드 실행·만료·완료 확인. Release 컴파일과 비격리
      단위 테스트만으로 IOS-4/-7/-8을 닫지 않는다.
- [ ] 양 앱 비행기모드 목소리 발사·잠금·재부팅/Doze, 계정 전환·가족 수신/ACK·철회 확인.
      A32 공휴일 계측 통과는 이 발사 체크리스트 전체의 통과가 아니다.
- [ ] 실제 스토어 구매/복원/보류/환불과 두 기기 권한 수렴 확인.
...
develop `948b5fdc`의 새 지적을 수정했다. iOS 일괄·단건 push는 소유자로 거르고,
귀속이 불확실한 레거시 행은 보류한다. 구독 조회는 재계산과 같은 유효 권한을 선택한다.
보류·미확인 구독의 초대 발급·재발급·사용과 무료 발신자의 음성 가족 알람을 차단한다.

- #741이 develop `d5893dde`에 머지돼 이번 네 지적의 구현이 반영됐다.
- [ ] 실기기 A→B 전환, 소유자 미기록 행과 미완료 로그아웃이 있는 상태에서 서버 계정 분리 확인
- [ ] dev 결제 보류→초대·전송 차단→복구, 복수 구독의 화면 등급·기능 게이트 일치 확인
- 로컬 빌드·테스트는 실행하지 않았다. 회귀 사례 작성 후 커밋·푸시하며 CI 완료는 기다리지 않는다.

원인과 재발 방지 범위는 [이번 4건 기록](pr-730-access-reviews-2026-09-14.md) 참조.

## #730 — #739 머지 후 iOS 동기화 리뷰 3건

develop `5bd7a23b`의 P1 1건·P2 2건을 수정했다. 수신 필터와 origin은 서버의
`is_received`(없으면 구형 표시)를 함께 따른다. 일괄·단건 전송은 요청 스냅샷과 현재 행을
비교해 동시 편집을 dirty로 보존하고, 생성 응답 ID를 디스크에 기록한 뒤 취소를 전파한다.

- [ ] 릴리스 단계: UUID·옛 로그인 식별자가 다른 계정의 가족 푸시 수신·예약·ACK 실기기 확인
- [ ] 실기기: 전송 중 편집 및 생성 성공 직후 BGTask 만료 뒤 재실행 시 ID·편집 내용 유지 확인
- 회귀 사례는 작성만 했다. 로컬 빌드·테스트·추가 리뷰 요청 없이 커밋·푸시하며 CI 완료는 기다리지 않는다.

상세는 [이번 리뷰 기록](pr-730-reviews-2026-09-14.md)의 「#739 머지 후」 참조.

## #739 후속 리뷰 — 보류 구독 제외·구서버 수신 표시

`ecd8c3b2`의 P2·P1 각 1건을 수정했다. 보류·미확인 구독은 DB 표식으로 이후 해지/환불의
등급 재계산에서도 제외한다. 정상 스토어 복구만 해당 소유자·그룹 멤버의 표식을 해제한다.
Android는 `is_received`가 없을 때 `is_received_family_alarm`로 수신 판별한다.

- `0282a42f` 원격 백엔드 테스트의 실패 4건은 Google 모의 함수 범위와 과거 프로모
  테스트 DB의 #118 누락을 수정했다. 수정 후 통과 여부는 다음 원격 CI에서 확인한다.
- [ ] 릴리스 단계: 마이그레이션 118 dev 적용 후 기존 스토어 구독·멤버의 unverified→스토어 재확인 확인
- [ ] 구서버 실제 응답의 받은 가족 알람이 Android에 임포트·예약되는지 실기기 확인
- 로컬 빌드·테스트·추가 리뷰 요청은 하지 않았다. CI 완료는 기다리지 않는다.

상세는 [이번 리뷰 기록](pr-730-reviews-2026-09-14.md)의 「#739 후속」 참조.

## #730 — #738 머지 후 새 리뷰 4건

develop `0315ee21`의 P1 1건·P2 3건을 수정했다. 그룹 복구·부분 해지 후에는 가장 높은 잔여
등급을 선택한다. 앱 선출시 중에는 양쪽 앱이 구서버의 유효한 offset 응답도 끝까지 읽는다.
iOS 일괄 push는 인스턴스가 달라도 호출별로 기다리며, 앞 작업 취소가 후속 전송을 버리지 않는다.
사이드키로 다른 앱이 화면을 덮는 경우의 스펙 문장도 계속 울림으로 맞췄다.

로컬 빌드·테스트·추가 리뷰 요청 없이 커밋·푸시한다. CI 완료는 기다리지 않는다.
원인·회귀 사례·남은 한계는 [이번 리뷰 기록](pr-730-reviews-2026-09-14.md)에 있다.

- [ ] 릴리스 단계에서 새 회귀 사례 실행 및 두 기기의 구서버/신서버 가족 알람 수신 확인
- [ ] 실제 iOS BGTask 만료와 전경 동기화 중첩 뒤 신규 로컬 편집 전송 확인

## #737 CI 실패 복구 — Swift 콜백의 actor 선언 누락

`7ea84109`의 iOS CI는 중복 교체 헬퍼의 비동기 콜백이 `MainActor` 밖으로 추론되어
컴파일에서 멈췄다. 함수와 콜백의 격리를 모두 명시해 `13e02f97`로 푸시했다.
로컬 빌드 제한을 원격 CI까지 잘못 확대해 취소했으나, 사용자가 CI는 허용하고
에이전트는 커밋·푸시 뒤 종료하라는 뜻으로 명확히 했다. 잘못 취소한 develop의
iOS·Android CI는 재실행을 요청했다. 완료는 기다리지 않는다.
이미 끝난 검사와 취소 경위는 [리뷰 대응 기록](pr-730-post-merge-reviews-2026-09-13.md)의
「#737 CI 실패 복구」에 기록한다. 실행 규칙은 `CLAUDE.md` 「컨벤션」을 따른다.

- 수정 HEAD의 원격 검사 상태는 [#737 체크](https://github.com/alpaka206/AlarmTalk/pull/737/checks)에서 확인한다.
- [ ] 아래의 실제 AlarmKit 예약/취소·가족 푸시 수신/ACK 실기기 확인은 별도로 남는다.

## #737 후속 3건 — 실제 푸시 진입점·발급 전 환불·401 교체 롤백

`f18d3da4`의 새 P1 2건·P2 1건을 수정했다. 전체 동기화 래퍼도 요청을 FIFO로 기다리고,
발급 전 환불은 Apple 구매자를 결제 원장에 연결한다. 교체 롤백은 현재 인증 대신 저장된
소유자·임시 편집본/예약이 같은지 확인해 자동 401 뒤에도 기존 알람을 복원한다.

- 자동 회귀 실행 결과는 위 CI 실패 복구 기록 참조.
- [ ] 기존 무연결 환불 표식이 있는 환경은 Apple 재검증으로 실제 구매자 연결 확인
      (옛 행에는 구매자·구매일이 없어 이번 코드만 배포해서 소급 복원할 수 없음)
- [ ] iOS 실기기: 콜드 로드 시간 초과 중 가족 푸시→뒤 회차 예약/ACK, 교체 삭제 401 뒤
      기존 예약 유지/신규 예약 회수, 명시적 로그아웃·동시 끄기 보존 확인

테스트·빌드·lint/typecheck·CI·추가 리뷰 요청은 실행하지 않았다. 상세는
[리뷰 대응 기록](pr-730-post-merge-reviews-2026-09-13.md)의 「#737 후속 3건」 참조.

## #737 추가 P2 — 탈퇴 뒤 Apple 선물 연결 잔존

`cf6ddc80` 대상 새 지적은 선물 코드·결제 원장을 파기해도 `apple_gift_deliveries`의
거래 ID가 남는 문제다. 즉시 삭제·유예 파기가 공유하는 `purgeUserAccount`에서 소유
원장·코드를 지우기 전에 연결을 삭제한다. 필요한 증빙은 기존 가명 보존/기한 정리를 따른다.

- [ ] 향후 실행 요청 시 `billing-apple-gift-refund.test.ts` 탈퇴 회귀 5건 실행
      (발급/환불/코드 선삭제/5년 경과, 타 계정·동일 문자열의 다른 스토어 거래 보존,
      원장 삭제 실패 시 전체 롤백)
- [ ] dev에서 마이그레이션 117 반영 뒤 즉시 삭제·유예 후 파기의 선물 연결 정리 확인

기존 마이그레이션·fingerprint는 변경하지 않았다. 테스트·빌드·lint/typecheck·CI·추가
리뷰 요청은 실행하지 않았다. 상세는 [리뷰 대응 기록](pr-730-post-merge-reviews-2026-09-13.md)의
「#737 추가 P2」 참조.

## #737 재후속 — 첫 pull 실패 뒤 대기 요청 유실

`72d60d9b` 대상 새 P1은 콜드 로드 시간 초과 전에 겹친 가족 푸시 요청을 잃는 문제다.
`runOnce`의 공용 불리언/재요청 표식을 호출별 FIFO 대기로 바꿨다. 앞 호출의 시간 초과·취소
후에도 뒤 호출이 자기 준비 대기와 최신 세션으로 실행되고 실제 회차 결과/오류를 받는다.
작업을 분리하여 백그라운드 취소를 회피하거나 무한히 재시도하지 않는다.

- [ ] 향후 실행 요청 시 `RemoteAlarmColdLoadTests`의 실제 `runOnce` 경합 회귀 2건 실행
      (첫 시간 초과/첫 취소, 대기 중 계정 변경, 뒤 회차 HTTP/디코딩/결과 확인)
- [ ] 실기기: 콜드 로드를 지연하고 가족 푸시 중첩→첫 회차 시간 초과→로드 완료 후
      주기 폴링을 기다리지 않고 대기 호출의 수신·예약·ACK가 실행되는지 확인

테스트·빌드·CI·추가 리뷰 요청은 실행하지 않는다. 상세는
[리뷰 대응 기록](pr-730-post-merge-reviews-2026-09-13.md)의 「#737 재후속」 참조.

## #737 후속 리뷰 — P1·P2 각 1건

- P1: 디스크 로드 시간 초과 뒤 pull의 오류 정리에서 OS/pending만 지우던 경로 수정.
  `retryPendingCancellations` 자체가 로드 전에는 정리를 보류하고, 로드 뒤 행의 핸들까지 정리한다.
- P2: Apple JWKS 강제 재조회에 kid/IP와 무관한 isolate 공통 30초 간격 추가.
  콜드/실패 조회도 포함하며 기존 정상 키는 계속 검증한다. 새 키는 간격 종료 후 재시도한다.
- [ ] 향후 실행 요청 시 `RemoteAlarmColdLoadTests` 후처리 2건과 JWKS 간격/실패/동시 요청 회귀 실행
- [ ] 실기기: 지연 로드→pull 시간 초과→로드 완료→pending 취소 정리→기존 알람 재예약 확인

테스트·빌드·CI·추가 리뷰 요청 없이 수정/회귀 사례/기록만 반영한다. 자세한 원인은
[리뷰 대응 기록](pr-730-post-merge-reviews-2026-09-13.md)의 「#737 후속」 참조.

## #730 — #736 머지 후 새 P1 5건·P2 1건

#736은 develop `c30bd26f`로 머지됐다. 그 HEAD의 새 6건을
`fix/pr-730-refund-and-ios-recovery`의 #737에 올렸다. 상세 원인·수정·한계는
[리뷰 대응 기록](pr-730-post-merge-reviews-2026-09-13.md)의 「#736 머지 후 새 6건」에 있다.

- Apple 소모성 선물 환불 분리·미사용 코드 만료·환불 선도착 재발급 차단.
- Apple JWKS 새 kid 재조회·APNs 설정 오류 시 정상 등록 보존.
- iOS 중복 교체 실패 시 기존 행 보존/새 예약 롤백·재예약 요청 FIFO 대기·디스크 로드 후 수신.
- [ ] 향후 실행 요청 시 새 백엔드/iOS 회귀 사례 실행(작성만 완료, 통과 여부 미확인)
- [ ] dev에서 마이그레이션 **117** 적용 확인 후 prod 순서 유지; 운영 DB 변경은 하지 않음
- [ ] 과거 선물의 미매핑 결제가 있다면 근거로 코드 연결 확인(불명확한 연결은 502로 보류)
- [ ] Apple 샌드박스: 미사용 선물 환불·재전송·이미 사용한 선물/다른 결제 보존 확인
- [ ] iOS 실기기: 서버 삭제 실패/다중 충돌/OS 취소 실패/교체 중 종료의 기존·새 알람 상태 확인
- [ ] iOS 실기기: 재예약 중 목소리 삭제/권한 변경·취소/새 요청에서 최종 예약 음원 확인
- [ ] iOS 실기기: 콜드 스타트 가족 푸시의 디스크 로드→수신→예약→ACK 및 오프라인 울림 확인
- [ ] APNs topic/환경 오류 뒤 설정 복구 시 기존 토큰으로 재전송 확인

사용자 지시에 따라 테스트·빌드·lint/typecheck·CI·추가 리뷰 요청은 하지 않는다.
수정 PR을 develop에 머지하기 전에는 #730의 코드/스레드를 해결 완료로 표시하지 않는다.

## #730 — #735 머지 후 P1·P2 각 1건 (수정은 #736으로 머지)

#735는 develop `466b25ac`로 머지됐고, 그 HEAD의 새 리뷰 두 건은 #736으로 반영됐다.
...
- [ ] 백엔드·마이그레이션 116 반영 뒤 양쪽 앱의 offset 호환→커서 전환 확인
...
- [ ] 전달 세대 커서 지원 서버·마이그레이션 116 완료 후 양쪽 앱의 커서 전환 확인
      (앱 선출시 기간에는 유효한 구서버 offset 응답 호환, 서버 쓰기의 옛 스키마 폴백은 없음)
...
- **App Store 배지**: Google Play 옆에 같은 무게로 그린다. 게재 전에는 '곧 출시' 로 죽은 링크
  ...
  (`lib/site.ts`). FAQ 기기 답변·`llms.txt` 는 게재 전까지 'iOS 준비 중' 이었고, **2026-09-22 게재로 두 스토어 모두 '지금 받을 수 있다' 로 바꿨다**(세 언어 + `llms.txt`).
...
- App Store 배지는 2026-09-15 부터 **기본 링크 활성**(심사 제출과 함께)이고, 2026-09-22 게재로 그 링크가 실제로 산다. 스토어에서 내려가는 사고가 나면 Vercel 에 `NEXT_PUBLIC_APP_STORE_LIVE=0` 추가 후 재배포.
...
- [x] **잠금 화면에서 전체화면**(2026-09-22 수정·확인): 잠긴 채로 울리면 울림 화면이 **즉시**
      뜨는가. S23(Android 16) 10:18 알람 → OS `usagestats` `ACTIVITY_RESUMED 10:18:00`,
      A32(Android 13) 10:26 알람 → 10:26:01, 인스턴스 하나. **수정 전에는 양쪽 다 소리만 났다**
      (S23 09:52 알람 → 화면은 잠금을 푼 09:52:12 에야, logcat `BAL_BLOCK`).
- [x] **앱을 쓰는 중이면 전체화면**: A32 10:30 알람, 앱 전경 → `RingingActivity` 즉시 표시.
- [ ] **다른 앱을 쓰는 중이면 배너**: 잠금 해제 + 다른 앱 전경에서 배너(끄기·다시 울림)가 뜨고,
      탭하면 전체 화면이 열리는가. S23 10:17 알람에서 배너의 '끄기' 로 해제된 것은 확인했고
      **탭해서 화면이 열리는 것**은 아직 안 봤다. 전체화면이 아닌 것은 OS 가 정한 것이라 정상이다.
- [x] **배너 겹침 없음**: 앱을 쓰는 중에 울리면 QUIET(LOW) 알림이라 울림 화면 위에 배너가 겹치지
      않는다. **잠금 화면 전체화면은 HIGH 채널이 열므로 채널을 강등하지 말 것.**
